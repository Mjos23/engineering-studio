...
3. Serve the static Studio over HTTPS, or use the documented loopback HTTP server while developing it. An actual browser must fetch the lesson JSON; opening the HTML file directly is insufficient. The UI remains browser-based; compilation runs on GitHub's hosted runner.
